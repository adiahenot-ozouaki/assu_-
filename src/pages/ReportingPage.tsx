...
const TooltipContent = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-surface-2 border border-border rounded-xl shadow-lg p-3 text-sm">
      <p className="font-semibold text-ink mb-2">{label}</p>
      {payload.map((p: any) => (
        <div key={p.name} className="flex items-center gap-2">
          <div className="w-2.5 h-2.5 rounded-full" style={{ background: p.color }} />
          <span className="text-ink-muted">{BRANCH_LABELS[p.name] ?? p.name} :</span>
          <span className="font-semibold text-ink">{fmtK(p.value)} FCFA</span>
        </div>
      ))}
    </div>
  );
...
function KpiCard({ label, value, sub, icon, trend, color = 'green' }: {
  ...
  const trendColor = trend === 'up' ? 'text-emerald-500' : trend === 'down' ? 'text-red-500' : 'text-ink-subtle';
    ...
    <Card className="p-5">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <p className="mt-1.5 text-2xl font-bold text-ink leading-none font-display">{value}</p>
          {sub && <p className="mt-1 text-xs text-ink-subtle">{sub}</p>}
        </div>
        <div className="flex flex-col items-end gap-2">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl ${colors[color]}`} aria-hidden>
            {icon}
          </div>
          {trend && <TrendIcon size={14} className={trendColor} aria-hidden />}
        </div>
      </div>
    </Card>
  );
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-ink font-display">Reporting Direction</h1>
        ...
        <button
          ...
          onClick={load}
          disabled={loading}
          className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-ink-muted bg-surface-2 border border-border rounded-lg hover:bg-surface-3 transition-all disabled:opacity-50"
        >
      ...
      <div>
        ...
        <Card className="p-5">
          {primes.length === 0 ? (
            <div className="h-64 flex items-center justify-center text-ink-subtle text-sm">
                ...
                {pieData.map(d => (
                  <div key={d.name} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-ink-muted">{d.name}</span>
                    </div>
                    <div className="text-right">
                      <span className="text-sm font-bold text-ink">{d.value}</span>
                      <span className="text-xs text-ink-subtle ml-1">contrats</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div>
          ...
          <Card className="p-5">
            {sinistraliteData.length === 0 ? (
              <p className="text-sm text-ink-subtle text-center py-8">Aucune donnée</p>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div>
          ...
          <Card className="p-5 text-center">
            <ResponsiveContainer width="100%" height={160}>
              <RadialBarChart cx="50%" cy="80%" innerRadius="60%" outerRadius="100%" startAngle={180} endAngle={0} data={radialData}>
                <RadialBar dataKey="value" cornerRadius={8} background={{ fill: 'rgb(var(--border))' }} />
              </RadialBarChart>
            </ResponsiveContainer>
            <div className="-mt-8">
              <p className="text-3xl font-bold text-ink font-display">{tauxRecouv}%</p>
        ...
        <div>
          ...
          <Card className="p-5 space-y-3">
            {modes.length === 0 ? (
              <p className="text-sm text-ink-subtle text-center py-6">Aucune donnée</p>
            ) : modes.map(m => {
              ...
              return (
                <div key={m.mode}>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-ink-muted">{MODE_LABELS[m.mode] ?? m.mode}</span>
                    <span className="font-semibold text-ink">{pct}% · {m.count}</span>
                  </div>
              ...
              {topClients.map((c, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3 px-4 py-3">
                    <div className="w-7 h-7 rounded-full bg-brand-soft flex items-center justify-center text-xs font-bold text-brand-dark shrink-0">
                      {i + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-ink truncate">{nom}</p>
                      <p className="text-xs text-ink-subtle">{c.client?.code_client}</p>
                    </div>
              ...
              <thead>
                <tr className="bg-navy text-white">
                  {['Branche', 'Contrats actifs', 'Sinistres', 'Déclaré', 'Indemnisé', 'Primes collectées', 'Taux S/P', 'En cours'].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider opacity-70 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
                    ...
                    <tr key={i} className="hover:bg-surface-3 transition-colors">
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-ink">{BRANCH_LABELS[s.branche] ?? s.branche}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5 text-ink-muted">{s.contrats_actifs}</td>
                      <td className="px-4 py-3.5 text-ink-muted">{s.nb_sinistres}</td>
                      <td className="px-4 py-3.5 text-ink-muted">{formatCurrency(s.montant_declare_total)}</td>
                      <td className="px-4 py-3.5 text-ink-muted">{formatCurrency(s.montant_indemnise_total)}</td>
                      <td className="px-4 py-3.5 text-ink-muted">{formatCurrency(s.prime_totale_branche)}</td>
                      <td className="px-4 py-3.5">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold ${
                          taux > 60 ? 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-300' :
                          taux > 30 ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300' :
                      ...
                      <td className="px-4 py-3.5">
                        {Number(s.sinistres_ouverts) > 0
                          ? <span className="font-semibold text-amber-600 dark:text-amber-400">{s.sinistres_ouverts}</span>
